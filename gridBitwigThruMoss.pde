  ...
  if ( updateGrid || update ) {
    ...
    for (int row = 0; row<= 7; row++) {
      for (int col = 0; col<=15; col++) { 
        ...
        int pRow = (int) map(col, 0,15, width * 0.1 , width * 0.9 );
        int pCol = (int) map(row, 0, 7, height * 0.1, height * 0.9);
...
void sendToAllTracks( String msg, int value) {
...
boolean skipTheseAddresses( OscMessage msg, String[] list) {
 for ( String s : list) { 
